...
export function AppShell({ userName }: { userName: string }) {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f17] text-slate-100">
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-[264px] border-r border-slate-800/80 bg-[#090d16] text-slate-100 transition-transform lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="flex h-20 items-center gap-3 border-b border-slate-800/80 px-6">
          <div className="grid h-10 w-10 place-items-center rounded-xl bg-emerald-500 text-slate-950 font-black shadow-[0_0_20px_rgba(16,185,129,0.35)]">
            <Bike size={23} strokeWidth={2.4} />
          </div>
          <div>
            <p className="text-[17px] font-extrabold tracking-[-0.03em] text-slate-100">
              모토웍스 AI
            </p>
            <p className="text-xs font-mono text-emerald-400/90 tracking-wide">PRECISION COCKPIT</p>
          </div>
        </div>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold transition ${active ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.15)] font-bold' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-200'}`}
                    >
                      <Icon size={18} className={active ? 'text-emerald-400' : 'text-slate-500'} />
                      <span className="flex-1">{item.label}</span>
                      {item.id === 'review' && pending > 0 && (
                        <span className="grid min-w-6 place-items-center rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 px-1.5 py-0.5 text-[11px] font-mono font-black shadow-[0_0_10px_rgba(245,158,11,0.25)]">
                          {pending}
                        </span>
                      )}
                    </button>
        ...
        <div className="absolute inset-x-3 bottom-3 rounded-xl border border-slate-800/80 bg-slate-900/60 p-3 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            <div className="grid h-9 w-9 place-items-center rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-sm font-black">
              김
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-slate-200">{userName}</p>
              <p className="text-xs text-slate-400">조직 관리자 · 전체 지점</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="min-h-screen pb-[calc(5rem+env(safe-area-inset-bottom))] lg:pb-0 lg:pl-[264px]">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-slate-800/80 bg-[#0b0f17]/85 px-3 backdrop-blur-xl sm:h-20 sm:gap-4 sm:px-7 lg:px-10">
          <Button
            variant="ghost"
            size="icon"
          ...
          <div className="min-w-0 flex-1">
            <h1 className="truncate text-lg font-black tracking-[-0.025em] text-slate-100 sm:text-2xl">
              {titles[view].title}
            </h1>
          ...
          <div className="hidden items-center gap-2 rounded-full border border-slate-700/80 bg-slate-900/80 px-3 py-2 text-sm text-slate-300 md:flex">
          ...
          <Button
            ...
            className="min-h-11 rounded-xl bg-emerald-500 text-slate-950 font-bold hover:bg-emerald-400 glow-emerald px-3 sm:px-4"
          >
            <CloudUpload size={17} />
        ...
        {notice && (
          <div className="mx-4 mt-4 flex items-center justify-between rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-3 text-sm font-semibold text-emerald-300 sm:mx-7 lg:mx-10 glow-box-emerald">
            <span>{notice}</span>
            <button onClick={() => setNotice('')} aria-label="알림 닫기" className="text-emerald-400 hover:text-emerald-200">
              <X size={16} />
            </button>
          </div>
        )}
        ...
        <nav
          ...
          className="safe-bottom fixed inset-x-0 bottom-0 z-30 grid grid-cols-4 border-t border-slate-800/90 bg-[#090d16]/95 px-2 pt-1.5 shadow-[0_-8px_30px_rgba(0,0,0,.6)] backdrop-blur-xl lg:hidden"
        >
          {[
            ...
            return (
              <button
                ...
                className={`relative flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl text-[12px] font-bold transition ${active ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/30' : 'text-slate-400 hover:text-slate-200'}`}
              >
                <Icon size={20} strokeWidth={active ? 2.5 : 2} />
                <span>{item.label}</span>
                {item.id === 'review' && pending > 0 && (
                  <span className="absolute right-[22%] top-1 grid min-h-4 min-w-4 place-items-center rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 px-1 text-[10px] font-black font-mono shadow-[0_0_8px_rgba(245,158,11,0.3)]">
                    {pending}
                  </span>
                )}
              </button>
    ...
    <div className="space-y-7">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.45fr_.75fr]">
        <section className="panel p-5 sm:p-6">
          <div className="mb-6 flex items-start justify-between gap-4">
            ...
            <Button
              variant="outline"
              className="rounded-xl border-slate-700 text-slate-300 hover:border-emerald-500 hover:text-emerald-400"
              onClick={() => navigate('review')}
            >
          ...
          <div className="space-y-3">
            {[
              ...
              <div
                key={String(label)}
                className="flex items-center gap-4 rounded-xl border border-slate-800/90 bg-slate-900/60 p-4 transition hover:border-slate-700 hover:bg-slate-900"
              >
                <span
        ...
        <section className="panel overflow-hidden">
          ...
          <div className="p-5 sm:p-6">
            ...
            <Button
              variant="outline"
              className="mt-6 w-full rounded-xl border-slate-700 text-slate-300 hover:border-emerald-500 hover:text-emerald-400"
      ...
      <section className="panel overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-800 p-5 sm:p-6">
          ...
          <Button variant="ghost" className="text-slate-400 hover:text-emerald-400 hover:bg-slate-800" onClick={() => navigate('orders')}>
...
function Metric({
  ...
  const tones: Record<string, string> = {
    amber: 'bg-amber-500/15 text-amber-400 border border-amber-500/30 glow-box-amber',
    green: 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 glow-box-emerald',
  ...
  return (
    <button
      onClick={onClick}
      className="panel group min-h-[150px] p-4 text-left transition duration-200 hover:-translate-y-1 hover:border-slate-700 hover:shadow-[0_16px_36px_rgba(0,0,0,0.5)] sm:min-h-0 sm:p-5"
    >
      <div className="mb-5 flex items-center justify-between">
        ...
        {onClick && <ChevronRight size={18} className="text-slate-600 group-hover:text-emerald-400 transition" />}
      </div>
      <p className="text-xs font-semibold text-slate-400 tracking-wide uppercase">{label}</p>
      <p className="mt-1 text-[22px] font-black tracking-[-.04em] text-slate-100 font-mono tabular-nums sm:text-[28px]">{value}</p>
      <p className="mt-2 line-clamp-2 text-xs leading-4 text-slate-500">{note}</p>
    </button>
  );
}
...
function Review({
  ...
  return (
    <div className="grid min-h-[680px] overflow-hidden rounded-2xl border border-slate-800 bg-[#0d131f] shadow-[0_16px_40px_rgba(0,0,0,0.5)] xl:grid-cols-[330px_1fr]">
      <aside className="border-b border-slate-800 bg-[#090d16] xl:border-b-0 xl:border-r">
        <div className="border-b border-slate-800 p-4">
          <div className="relative">
            ...
            <Input
              ...
              className="rounded-xl border-slate-700 bg-slate-950 text-slate-100 placeholder:text-slate-500 pl-9 focus:border-emerald-500"
            />
          </div>
          <div className="mt-3 flex gap-2">
            <Badge variant="secondary" className="bg-amber-500/20 text-amber-300 border border-amber-500/40 glow-box-amber">
              대기 {pendingDocs.length}
            </Badge>
            <Badge variant="outline" className="border-slate-700 text-slate-400">낮은 신뢰도 우선</Badge>
          </div>
        </div>
          ...
          {pendingDocs.map((document) => {
            ...
            return (
              <button
                ...
                className={`min-w-[250px] snap-start rounded-xl border p-4 text-left transition xl:w-full xl:min-w-0 xl:rounded-none xl:border-x-0 xl:border-t-0 ${isSelected ? 'bg-emerald-500/10 border-l-2 border-l-emerald-400 border-slate-800 text-slate-100' : 'bg-slate-900/30 border-slate-800/80 text-slate-300 hover:bg-slate-800/50'}`}
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="rounded-full bg-amber-500/20 px-2 py-1 text-[11px] font-black font-mono text-amber-300 border border-amber-500/40">
                    {issues}개
                  </span>
                </div>
                <div className="mt-3 flex items-center justify-between text-xs">
                  <span className="text-slate-400">{document.shopName}</span>
                  <strong className="font-mono tabular-nums text-emerald-400">{won.format(document.amount)}</strong>
                </div>
              </button>
            );
          })}
      ...
      <section className="min-w-0">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 bg-[#0d131f] px-5 py-4">
          <div>
            <p className="text-sm font-extrabold text-slate-100">{selected.fileName}</p>
            <p className="text-xs text-slate-400 font-mono">{selected.id} · 2026-09-06</p>
          </div>
          <div className="hidden gap-2 sm:flex">
            ...
            <Button
              className="rounded-xl bg-emerald-500 text-slate-950 font-bold hover:bg-emerald-400 glow-emerald"
        ...
        <div className="grid lg:grid-cols-[minmax(300px,.9fr)_minmax(360px,1.1fr)]">
          ...
          <div className="p-4 sm:p-6 bg-[#0d131f]">
            <div className="mb-5 flex items-center justify-between">
              <div>
                <p className="eyebrow">AI 판독</p>
                <h2 className="section-title">확인이 필요한 항목</h2>
              </div>
              <Badge className="bg-amber-500/20 text-amber-300 border border-amber-500/40 glow-box-amber">
                {
            ...
            <div className="space-y-4">
              {selected.fields.map((field) => {
                ...
                return (
                  <div
                    key={field.id}
                    className={`rounded-xl border p-4 transition ${highlight ? 'border-amber-500/40 bg-amber-500/5 glow-box-amber' : 'border-slate-800 bg-slate-900/60'}`}
                  >
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <span
                        className={`text-xs font-black font-mono ${field.confidence >= 0.96 ? 'text-emerald-400' : 'text-amber-400'}`}
                      >
                    ...
                    <Input
                      ...
                      className={`h-11 rounded-lg bg-slate-950 font-mono tabular-nums text-slate-100 ${highlight ? 'border-amber-500/60 focus:border-amber-400' : 'border-slate-700 focus:border-emerald-500'}`}
                      placeholder="확인 후 입력"
                    />
                    {highlight && (
                      <div className="mt-2 flex gap-2 text-xs leading-5 text-amber-300">
        ...
        <div className="sticky bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-20 grid grid-cols-2 gap-2 border-t border-slate-800 bg-[#0b0f17]/95 p-3 shadow-[0_-8px_30px_rgba(0,0,0,.6)] backdrop-blur sm:hidden">
          <Button
            variant="outline"
            className="min-h-12 rounded-xl border-rose-500/30 text-rose-400 hover:bg-rose-500/10 hover:text-rose-300"
            onClick={() => resolve('rejected')}
            disabled={resolving}
          >
            <X size={17} /> 반려
          ...
          <Button
            className="min-h-12 rounded-xl bg-emerald-500 text-slate-950 font-bold hover:bg-emerald-400 glow-emerald"
            onClick={() => resolve('approved')}
            disabled={resolving}
          >
...
function DocumentPreview({ document }: { document: ReviewDocument }) {
  return (
    <div className="border-b border-slate-800 bg-[#070a0f] p-4 lg:border-b-0 lg:border-r lg:p-5">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm font-extrabold text-slate-200">원본 사진 (OCR 검사기)</p>
        <span className="text-xs font-mono font-semibold text-emerald-400">100% SCALE</span>
      </div>
      <div className="relative mx-auto aspect-[3/4] max-h-[380px] max-w-[285px] overflow-hidden rounded-lg border border-slate-800 bg-[#0b0f17] shadow-[0_16px_40px_rgba(0,0,0,.6)] sm:max-h-[520px] sm:max-w-none">
        {document.sourceUrl ? (
          <Image
            ...
            className="bg-[#0b0f17] object-contain"
          />
        ) : (
        <div className="flex h-full flex-col p-[8%] bg-[#0d131f] text-slate-200">
          <div className="border-b-2 border-emerald-500 pb-3 text-center text-xl font-black tracking-[.16em] text-emerald-400">
            정 비 내 역 서
          </div>
          <div className="mt-5 grid grid-cols-[90px_1fr] border border-slate-700 text-[11px] leading-8">
            <span className="border-b border-r border-slate-700 px-2 font-bold text-slate-400">정비일</span>
            <span className="border-b border-slate-700 px-2 font-mono">9 / 3</span>
            <span className="border-b border-r border-slate-700 px-2 font-bold text-slate-400">고객명</span>
            <span className="border-b border-slate-700 px-2">
              {document.customerName}
            </span>
            <span className="border-b border-r border-slate-700 px-2 font-bold text-slate-400">차량</span>
            <span className="border-b border-slate-700 px-2">
              {document.vehicleLabel}
            </span>
            <span className="border-r border-slate-700 px-2 font-bold text-slate-400">매장</span>
            <span className="px-2">{document.shopName}</span>
          </div>
          <div className="mt-6 flex-1 space-y-3 border-y border-slate-700 py-4 text-sm font-mono">
            <p>
              •{' '}
              {
            ...
            <p>• 점검 및 조정</p>
            <p className="text-right text-base font-bold text-emerald-400">
              합계 {document.amount.toLocaleString()}원
            </p>
          </div>
          <p className="mt-4 text-center text-xs font-bold text-slate-500">안전 운행하세요</p>
        </div>
        )}
        {document.fields
          ...
          .map((field, index) => (
            <span
              key={field.id}
              className="absolute rounded border-2 border-emerald-400 bg-emerald-400/20 glow-box-emerald"
              style={{
                left: `${18 + index * 8}%`,
        ...
        {!document.sourceAvailable && (
          <div className="absolute inset-x-4 bottom-4 rounded-lg bg-[#0b0f17]/95 border border-slate-800 px-3 py-2 text-center text-xs font-semibold text-slate-300">
            실제 원본 미수신 · Mock 미리보기
          </div>
        )}
      </div>
...
function PaymentPart({ method, amount }: { method: string; amount: number }) {
  return (
    <div className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/60 p-3">
      ...
      <strong className="font-mono tabular-nums text-emerald-400 font-bold">{won.format(amount)}</strong>
    </div>
  );
}
function StatusBadge({
  ...
  return (
    <span
      className={`inline-flex whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-bold font-mono tracking-tight ${tones[tone]}`}
    >
      {label}
    </span>
  );
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-slate-800/80 py-2.5 text-sm last:border-0">
      <dt className="text-slate-400">{label}</dt>
      <dd className="max-w-[70%] text-right font-bold font-mono tabular-nums text-slate-100">{value}</dd>
    </div>
  );
}
function EmptyState({ title, text }: { title: string; text: string }) {
  return (
    <div className="grid min-h-[420px] place-items-center p-8 text-center">
      <div>
        <div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 glow-box-emerald">
          <Check size={26} />
        </div>
        <h2 className="mt-5 text-xl font-extrabold text-slate-100">{title}</h2>
